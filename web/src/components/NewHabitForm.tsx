...
export function NewHabitForm() {
    ...
    function handleToggleWeekDay(weekDay: number) {
        if (!weekDays.includes(weekDay)) {
            const weekDaysWithNew = [...weekDays, weekDay];
            setWeekDays(weekDaysWithNew);
        } else {
            setWeekDays(weekDays.filter(day => day !== Number(weekDay)));
        }

    }

    return (
        <form onSubmit={submitHabit} className="w-full flex flex-col mt-6">
            ...
            <input
                ...
                className="p-4 rounded-lg mt-3 mb-3 bg-zinc-800 text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
                autoFocus
                value={title}
            ...
            <button
                type="submit"
                className="mt-6 rounded-lg p4 flex items-center justify-center gap-3 font-semibold bg-green-600 hover:bg-green-500 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-zinc-900">
